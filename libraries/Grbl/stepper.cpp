...
void StepperMotor::init(uint32_t pin1, uint32_t pin2, uint32_t pin3, uint32_t pin4) {
  _p1 = pin1; _p2 = pin2; _p3 = pin3; _p4 = pin4; _pos = 0;
  if(_p1>0) pinMode(_p1, OUTPUT);
  if(_p2>0) pinMode(_p2, OUTPUT);
  if(_p3>0) pinMode(_p3, OUTPUT);
  if(_p4>0) pinMode(_p4, OUTPUT);
}
void StepperMotor::idle() {
  setPinLow(_p1); setPinLow(_p2); setPinLow(_p3); setPinLow(_p4);
}
void StepperMotor::step(int forward) {
  _pos = forward>0? (_pos+1):(_pos-1);
  _pos &= 0x03;
  switch(_pos)
  {
  case 0: // 1010
    setPinHigh(_p1); setPinLow(_p2); setPinHigh(_p3); setPinLow(_p4);
    break;
  case 1: // 0110
    setPinLow(_p1); setPinHigh(_p2); setPinHigh(_p3); setPinLow(_p4);
    break;
  case 2: // 0101
    setPinLow(_p1); setPinHigh(_p2); setPinLow(_p3); setPinHigh(_p4);
    break;
  case 3: // 1001
    setPinHigh(_p1); setPinLow(_p2); setPinLow(_p3); setPinHigh(_p4);
    break;
  }
}
